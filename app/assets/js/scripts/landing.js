...
async function dlAsync(login = true) {
    ...
    // Auto-clean untracked mods only (preserving custom shaders, resourcepacks, configs)
    try {
        const localModsDir = path.join(ConfigManager.getInstanceDirectory(), serv.rawServer.id, 'mods')
        if (fs.existsSync(localModsDir)) {
            const officialMods = new Set(
                serv.modules
                    .filter(m => m.rawModule.path && m.rawModule.path.startsWith('mods/'))
                    .map(m => path.basename(m.rawModule.path).toLowerCase())
            )
            const localFiles = fs.readdirSync(localModsDir)
            for (const file of localFiles) {
                if (file.endsWith('.jar') && !officialMods.has(file.toLowerCase())) {
                    loggerLaunchSuite.info(`Auto-clean: Removing untracked/removed mod ${file}`)
                    try {
                        fs.removeSync(path.join(localModsDir, file))
                    } catch (e) {
                        loggerLaunchSuite.error(`Failed to remove untracked mod ${file}`, e)
                    }
                }
            }
        }
    } catch (cleanErr) {
        loggerLaunchSuite.warn('Error during auto-cleaning untracked mods:', cleanErr)
    }
