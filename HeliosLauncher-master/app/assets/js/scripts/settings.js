...
async function resolveDropinModsForUI(){
    ...
    CACHE_DROPIN_MODS = DropinModUtil.scanForDropinMods(CACHE_SETTINGS_MODS_DIR, serv.rawServer.minecraftVersion).filter(d => !serv.modules.some(m => m.rawModule.artifact && m.rawModule.artifact.path && (m.rawModule.artifact.path.endsWith(d.fullName) || m.rawModule.artifact.path.endsWith(d.name))))
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/puppet04/VelvetLauncher/releases.atom',
        success: (data) => {
